...
import { useWhatsAppConfig } from "@/app/hooks/useWhatsAppConfig";
...
const Navbar = () => {
  ...
  const { config: whatsAppConfig, isLoading: isWhatsAppConfigLoading } = useWhatsAppConfig();
  ...
  const menuMainItems = useMemo(() => [
    ...
    { 
      label: 'WhatsApp',
      href: whatsAppConfig?.phoneNumber ? `https://wa.me/${whatsAppConfig.phoneNumber}` : '#',
      icon: <FaWhatsapp className="inline-block mr-1" size={16} />
    },
