  ...
  const generateCode = () => {
    ...
    if (endpoint.id === 'session') {
      if (language === 'js') {
        ...
        return `# Create a new session\nimport requests\n\nurl = 'https://api.memoryorchestrator.com/api/sessions'\nheaders = {\n    'Content-Type': 'application/json',\n    'Authorization': 'Bearer YOUR_API_KEY'\n}\npayload = {\n    'name': 'Customer Support Session',\n    'ttl': 3600\n}\nresponse = requests.post(url, headers=headers, json=payload)\nsession = response.json()\nsession_id = session['id']  # Use this for subsequent queries`;
      } else if (language === 'curl') {
        return `# Create a new session\ncurl -X POST https://api.memoryorchestrator.com/api/sessions \\n  -H "Content-Type: application/json" \\\n  -H "Authorization: Bearer YOUR_API_KEY" \\\n  -d '{"name": "Customer Support Session", "ttl": 3600}'`;
      }
    }
    ...
    if (endpoint.id === 'context') {
      if (language === 'js') {
        ...
        return `# Preview context for a query\nimport requests\n\nquery = "What is our current project timeline?"\nurl = f'https://api.memoryorchestrator.com/api/context?query={requests.utils.quote(query)}&sessionId=session_12345'\nheaders = {\n    'Authorization': 'Bearer YOUR_API_KEY'\n}\nresponse = requests.get(url, headers=headers)\npreview = response.json()\nprint(preview['contextItems'])  # Array of context items that would be used`;
      } else if (language === 'curl') {
        return `# Preview context for a query\ncurl -X GET "https://api.memoryorchestrator.com/api/context?query=What%20is%20our%20current%20project%20timeline%3F&sessionId=session_12345" \\\n  -H "Authorization: Bearer YOUR_API_KEY"`;
      }
    }
